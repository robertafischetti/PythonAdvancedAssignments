...
""" HEADER
    ...
    1.1 Re-organizing the imports, dividing them into 3 sections:
    - Python API
    - 3rd party API
    - Custom API

    1.2 Re-ordering them by character length
...
""" FUNCTIONS and CLASSES
    ...
    2.1 Separating functions' categories using comments
    2.2 Adding empty lines
    2.3 Using a for loop instead of the following dictionary comprehension
    2.4 adding return annotations
    2.5 splitting long lines of text into multiple lines
...
class ArLoad(ArUtil):
    ...
    def __init__(self):
        ...
        for x, y in self.data['software']['EXTENSION'].items():
            self.software_format[y] = x.upper()
    ...
    def press_menuItemAddFolder(self) -> None:
        import arSaveAs
        self.save_as = arSaveAs.start(new_file=False)

    def press_menuSort(self, list_widget, reverse=False) -> None:
        file_list = []
    ...
    def fill_meta(self) -> None:
        ...
        self.wgPreview.lblDate.setText(
            str(datetime.datetime.fromtimestamp(os.path.getmtime(self.load_file))).split(".")[0]
            )
        self.wgPreview.lblSize.setText(
            str("{0:.2f}".format(os.path.getsize(self.load_file)/(1024*1024.0)) + " MB")
            )
